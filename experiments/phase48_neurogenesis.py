"""
Phase 48: Structural Super-Compensation (Neurogenesis)
========================================================
Instead of DESTROYING 15% of neurons (ablation), what if we
ADD new neurons at test time?

Approach:
  At test time, dynamically expand the hidden dimension by adding
  zero-initialized "growth channels." Then adapt for 50 steps.
  The new neurons start as blank slate and can learn task-specific
  features without overwriting existing knowledge.

Compare:
  - Ablation (destroy 15%, adapt 50) = "destructive plasticity"
  - Neurogenesis (add 15%, adapt 50) = "constructive plasticity"
  - Both (ablate 15% + add 15%, adapt 50) = "creative destruction"
"""
...
def add_noise_neurons(model, rate=0.15):
    """'Neurogenesis': add small random noise to a fraction of zero-valued
    or low-magnitude weights, simulating new synaptic connections."""
    am = copy.deepcopy(model)
    with torch.no_grad():
        for p in am.parameters():
            # Add small random perturbation to `rate` fraction of parameters
            mask = torch.rand_like(p) < rate
            noise = torch.randn_like(p) * 0.01  # small random init
            p.add_(mask.float() * noise)
    return am

def creative_destruction(model, rate, task_samples):
    """Ablate THEN grow: destroy old paths, add new potential connections."""
    am = ablate_least_important(model, rate, task_samples)
    am = add_noise_neurons(am, rate)
    return am
...
def main():
    print("=" * 60)
    print("Phase 48: Structural Super-Compensation")
    print("Destruction vs Construction vs Creative Destruction")
    print("=" * 60)
    t0 = time.time()
    ...
    configs = [
        ('adapt_only',   'Adapt 50 (baseline)',               'adapt'),
        ('ablate_15',    'Ablate 15% + Adapt (One Punch)',     'ablate'),
        ('neuro_5',      'Neurogenesis 5% + Adapt',            'grow_5'),
        ('neuro_15',     'Neurogenesis 15% + Adapt',           'grow_15'),
        ('neuro_30',     'Neurogenesis 30% + Adapt',           'grow_30'),
        ('creative_15',  'Creative Destruction 15% + Adapt',   'creative'),
    ]
    results = {}

    print(f"\n  {'Method':<40} | {'Full Match':>10}")
    print("  " + "-" * 55)

    for key, label, method in configs:
        ...
        for task in test_tasks:
            ...
            elif method.startswith('grow_'):
                rate = int(method.split('_')[1]) / 100.0
                grown = add_noise_neurons(model, rate)
                current = adapt_model(grown, aug_samples, steps=50)
            elif method == 'creative':
                cd = creative_destruction(model, 0.15, aug_samples)
                current = adapt_model(cd, aug_samples, steps=50)
        ...
        print(f"  {label:<40} | {rate:>9.1%} (delta: {delta:+.1%})")

    elapsed = time.time() - t0
    results['elapsed'] = elapsed
    results['timestamp'] = time.strftime('%Y-%m-%dT%H:%M:%S')

    with open(os.path.join(RESULTS_DIR, 'phase48_neurogenesis.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    fig, ax = plt.subplots(figsize=(14, 6))
    names = [c[1] for c in configs]
    vals = [results[c[0]] for c in configs]
    colors = ['#9E9E9E', '#E91E63', '#4CAF50', '#2196F3', '#00BCD4', '#9C27B0']
    ...
    ax.set_ylabel('Full Match'); ax.set_title('Phase 48: Structural Super-Compensation (Neurogenesis)')
    ax.set_ylim(0, 1)
    for b, v in zip(bars, vals):
        ax.text(b.get_x()+b.get_width()/2, v+0.02, f'{v:.1%}', ha='center', fontsize=11, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase48_neurogenesis.png'), dpi=150)
    plt.close()
