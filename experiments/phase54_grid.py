"""
Phase 54: The Grand Unified Grid (My own addition)
=====================================================
P50 found 14% ablation is optimal. P48 found 15% neurogenesis is optimal.
But those were found INDEPENDENTLY.

What if the optimal combination is something entirely different?
Full 2D grid search: ablation rate × neurogenesis rate.

This is the definitive landscape of neural perturbation.
"""
...
def main():
    print("=" * 60)
    print("Phase 54: The Grand Unified Grid")
    print("2D sweep: ablation rate x neurogenesis rate")
    print("=" * 60)
    t0 = time.time()
    ...
    # 2D grid
    abl_rates = [0.0, 0.10, 0.14, 0.18, 0.25]
    neuro_rates = [0.0, 0.05, 0.10, 0.15, 0.20]
    ...
    print(f"  *** Baseline (0%+0%): {baseline:.1%}, Delta: {best_val-baseline:+.1%} ***")
    ...
    with open(os.path.join(RESULTS_DIR, 'phase54_grid.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    # Heatmap
    fig, ax = plt.subplots(figsize=(10, 8))
    im = ax.imshow(grid * 100, cmap='RdYlGn', aspect='auto', vmin=60, vmax=100)
    ...
    ax.set_title(f'Phase 54: Grand Unified Grid (100 adapt steps)\nBest: A={best_abl:.0%} N={best_neuro:.0%} = {best_val:.1%}')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase54_grid.png'), dpi=150)
    plt.close()
