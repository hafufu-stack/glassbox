"""
Phase 56: Learning Rate Sensitivity (My addition)
=====================================================
We've used lr=1e-2 for ALL adaptation throughout the project.
With the new optimal perturbation (A=25%, N=10%), is that still best?

Stronger perturbation might need a different learning rate.
Test: lr=3e-3, 5e-3, 1e-2, 2e-2, 5e-2
"""
...
def main():
    print("=" * 60)
    print("Phase 56: Learning Rate Sensitivity")
    print("Does A=25%+N=10% need a different LR?")
    print("=" * 60)
    t0 = time.time()
    ...
    learning_rates = [3e-3, 5e-3, 1e-2, 2e-2, 5e-2]
    # Test with both: no perturbation and best perturbation
    configs = [
        ('baseline',  'No perturbation',       0,    0),
        ('optimal',   'A=25% + N=10%',         0.25, 0.10),
    ]
    results = {}

    print(f"\n  {'LR':>8} | {'No Perturb':>10} | {'A25+N10':>10} | {'Δ Perturb':>10}")
    print("  " + "-" * 48)

    for lr in learning_rates:
        row = {}
        for key, label, abl_r, neuro_r in configs:
            ...
            for task in test_tasks:
                ...
                current = adapt_model(current, aug_samples, steps=100, lr=lr)
                ok, tot = eval_task(current, task)
                ok_total += ok; n_total += tot
            rate = ok_total / max(n_total, 1)
            row[key] = rate
            results[f'{key}_lr{lr}'] = rate

        delta = row['optimal'] - row['baseline']
        print(f"  {lr:>8.0e} | {row['baseline']:>9.1%} | {row['optimal']:>9.1%} | {delta:>+9.1%}")

    elapsed = time.time() - t0
    results['elapsed'] = elapsed
    results['timestamp'] = time.strftime('%Y-%m-%dT%H:%M:%S')

    with open(os.path.join(RESULTS_DIR, 'phase56_lr.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    fig, ax = plt.subplots(figsize=(12, 6))
    lr_labels = [f'{lr:.0e}' for lr in learning_rates]
    base_vals = [results[f'baseline_lr{lr}'] for lr in learning_rates]
    opt_vals = [results[f'optimal_lr{lr}'] for lr in learning_rates]
    x = range(len(learning_rates))
    ax.plot(x, base_vals, 'o-', color='#9E9E9E', linewidth=2, markersize=8, label='No Perturbation')
    ax.plot(x, opt_vals, 's-', color='#E91E63', linewidth=2, markersize=8, label='A=25% + N=10%')
    ax.set_xticks(x); ax.set_xticklabels(lr_labels)
    ax.set_xlabel('Learning Rate'); ax.set_ylabel('Full Match')
    ax.set_title('Phase 56: Learning Rate Sensitivity')
    ax.legend(); ax.grid(True, alpha=0.3); ax.set_ylim(0, 1)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase56_lr.png'), dpi=150)
    plt.close()
