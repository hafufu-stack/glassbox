"""
Phase 44: Super-Compensation Distillation
=============================================
Can we bake the "super-compensated wisdom" into the base model
so we don't need ablation at test time?

Method:
  1. For each training task: run ablate 15% + adapt 50 steps
  2. Collect soft labels (logits) from the super-compensated model
  3. Train a NEW base model using knowledge distillation (KD)
  4. Test: does the distilled model beat the original WITHOUT any
     test-time adaptation or ablation?

This is "learning from your future, tougher self."
"""
...
def main():
    print("=" * 60)
    print("Phase 44: Super-Compensation Distillation")
    print("Learning from your future, tougher self")
    print("=" * 60)
    t0 = time.time()
    ...
    # ===== Step 2: Generate soft labels from super-compensated teachers =====
    print("\n  Generating soft labels from super-compensated teachers...")
    T = 2.0  # Temperature for distillation
    soft_labels = []  # list of (sample, soft_op, soft_c1, soft_c2, soft_ptr)
    ...
    for i, task in enumerate(train_tasks):
        ...
        for p in demos:
            ...
            with torch.no_grad():
                ...
                soft_op = F.softmax(ol / T, dim=-1).cpu()
                soft_c1 = F.softmax(cl1 / T, dim=-1).cpu()
                soft_c2 = F.softmax(cl2 / T, dim=-1).cpu()
                soft_ptr = F.softmax(pl / T, dim=-1).cpu()
            soft_labels.append((s, soft_op, soft_c1, soft_c2, soft_ptr))

        if (i + 1) % 50 == 0:
            print(f"    Processed {i+1}/{len(train_tasks)} tasks")

    print(f"  Collected {len(soft_labels)} soft-labeled samples")

    # ===== Step 3: Knowledge Distillation =====
    print("\n  Training distilled model...")
    student = Agent().to(DEVICE)
    opt_s = torch.optim.Adam(student.parameters(), lr=1e-3)
    alpha = 0.5  # balance: 0.5 hard labels + 0.5 soft labels

    for ep in range(80):
        student.train(); random.shuffle(soft_labels); el, nb = 0, 0
        for s, s_op, s_c1, s_c2, s_ptr in soft_labels:
            # Hard label loss
            hard_loss = compute_loss(student, s)

            # Soft label loss (KL divergence with teacher)
            ol, cl1, cl2, pl = student(s['nf'].to(DEVICE), s['nn'].to(DEVICE))
            kd_op = F.kl_div(F.log_softmax(ol / T, dim=-1), s_op.to(DEVICE), reduction='batchmean') * (T * T)
            kd_c1 = F.kl_div(F.log_softmax(cl1 / T, dim=-1), s_c1.to(DEVICE), reduction='batchmean') * (T * T)
            kd_c2 = F.kl_div(F.log_softmax(cl2 / T, dim=-1), s_c2.to(DEVICE), reduction='batchmean') * (T * T)
            kd_ptr = F.kl_div(F.log_softmax(pl / T, dim=-1), s_ptr.to(DEVICE), reduction='batchmean') * (T * T)
            soft_loss = (kd_op + kd_c1 + kd_c2 + kd_ptr)
    ...
    print("\n  --- Evaluation ---")
    methods = {
        'base_greedy': ('Original base model (greedy)', model, False, False),
        'base_adapt': ('Original + Adapt 50', model, True, False),
        'base_super': ('Original + Ablate 15% + Adapt 50', model, True, True),
        'distilled_greedy': ('Distilled model (greedy)', student, False, False),
        'distilled_adapt': ('Distilled + Adapt 50', student, True, False),
        'distilled_super': ('Distilled + Ablate 15% + Adapt 50', student, True, True),
    }
    results = {}

    for key, (label, m, do_adapt, do_ablate) in methods.items():
        ...
        results[key] = rate
        print(f"  {label:<42}: {rate:.1%}")

    elapsed = time.time() - t0
    results['elapsed'] = elapsed
    results['timestamp'] = time.strftime('%Y-%m-%dT%H:%M:%S')

    with open(os.path.join(RESULTS_DIR, 'phase44_distillation.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    # Plot
    fig, ax = plt.subplots(figsize=(14, 6))
    labels_plot = ['Base\nGreedy', 'Base\n+Adapt', 'Base\n+Super', 'KD\nGreedy', 'KD\n+Adapt', 'KD\n+Super']
    vals = [results[k] for k in ['base_greedy', 'base_adapt', 'base_super',
                                  'distilled_greedy', 'distilled_adapt', 'distilled_super']]
    colors = ['#9E9E9E', '#FF9800', '#2196F3', '#E91E63', '#4CAF50', '#673AB7']
    bars = ax.bar(range(6), vals, color=colors, alpha=0.85)
    ax.set_xticks(range(6)); ax.set_xticklabels(labels_plot, fontsize=10)
    ax.set_ylabel('Full Match'); ax.set_title('Phase 44: Super-Compensation Knowledge Distillation')
    ax.set_ylim(0, 1)
    for b, v in zip(bars, vals):
        ax.text(b.get_x()+b.get_width()/2, v+0.02, f'{v:.1%}', ha='center', fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase44_distillation.png'), dpi=150)
    plt.close()
