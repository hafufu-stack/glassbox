"""
Phase 40: The Full Pipeline — Triage + Ensemble Integration
==============================================================
The ultimate combination: Phase 38 (demo-verified triage) +
Phase 39 (ablation-diverse ensemble) in a single pipeline.

For each test task:
  1. Check base model on demos
     -> All correct? Use base model (skip)
  2. Full adapt 50 steps, re-check
     -> All correct? Use adapted model
  3. Ablation-Diverse Ensemble (K=5 ablated variants)
     -> Pick best via demo verification

This is the complete Antifragile Intelligence pipeline.
"""
...
def verify_on_demos(model, demo_samples):
    model.eval(); ok, total, total_loss = 0, 0, 0.0
...
def create_ablated_variants(model, rate, aug_samples, K=5):
    ...
    am.eval(); variants.append(('strategic', am))
    for i in range(1, K):
        am = copy.deepcopy(model)
...
def full_pipeline(model, task, aug_samples, demo_samples, K=5):
    """The complete Antifragile Pipeline:
    1. Base model check -> skip if perfect
    2. Full adapt -> use if perfect
    3. Ablation-Diverse Ensemble -> pick best
    """
    # Step 1: Base model
    ok, total, _ = verify_on_demos(model, demo_samples)
    if total > 0 and ok == total:
        return model, 'base_skip'

    # Step 2: Full adapt
    adapted = adapt_model(model, aug_samples, steps=50)
    ok2, total2, _ = verify_on_demos(adapted, demo_samples)
    if total2 > 0 and ok2 == total2:
        return adapted, 'adapt_pass'

    # Step 3: Ablation-Diverse Ensemble
    variants = create_ablated_variants(model, 0.15, aug_samples, K)
    best_model = adapted  # fallback
    best_ok = ok2
    best_loss = float('inf')
    best_variant = 'adapt_fallback'

    for name, ablated in variants:
        adapted_v = adapt_model(ablated, aug_samples, steps=50)
        ok_v, total_v, loss_v = verify_on_demos(adapted_v, demo_samples)
        if ok_v > best_ok or (ok_v == best_ok and loss_v < best_loss):
            best_ok = ok_v
            best_loss = loss_v
            best_model = adapted_v
            best_variant = f'ensemble_{name}'

    return best_model, best_variant
...
def main():
    print("=" * 60)
    print("Phase 40: The Full Antifragile Pipeline")
    print("Triage + Ensemble = The Conqueror")
    print("=" * 60)
    t0 = time.time()
    ...
    # Run baselines + full pipeline
    methods = [
        ('adapt_only', 'Adapt-Only (P28)'),
        ('ablate_15', 'Fixed Ablate 15% (P34)'),
        ('pipeline_K3', 'Full Pipeline K=3'),
        ('pipeline_K5', 'Full Pipeline K=5'),
    ]
    results = {}
    pipeline_stats = {}
    ...
    for method_key, label in methods:
        ...
        for task in test_tasks:
            ...
            if method_key == 'adapt_only':
                best = adapt_model(model, aug_samples, steps=50)
            elif method_key == 'ablate_15':
                am = copy.deepcopy(model); am.train()
                tl = torch.tensor(0.0, device=DEVICE)
                for s in aug_samples[:8]: tl = tl + compute_loss(am, s)
                tl = tl / max(len(aug_samples[:8]), 1); tl.backward()
                with torch.no_grad():
                    for p in am.parameters():
                        if p.grad is not None:
                            imp = p.grad.abs(); thr = torch.quantile(imp.flatten(), 0.15)
                            p.mul_((imp > thr).float())
                        else:
                            p.mul_((torch.rand_like(p) > 0.15).float())
                am.eval()
                best = adapt_model(am, aug_samples, steps=50)
            elif method_key.startswith('pipeline'):
                K = int(method_key.split('K')[1])
                best, level = full_pipeline(model, task, aug_samples, demo_samples, K=K)
        ...
        if level_counts:
            pipeline_stats[method_key] = level_counts
        delta = rate - results.get('adapt_only', rate)
        print(f"  {label:<30}: {rate:.1%} (delta: {delta:+.1%})")
        if level_counts:
    ...
    with open(os.path.join(RESULTS_DIR, 'phase40_pipeline.json'), 'w', encoding='utf-8') as f:
    ...
    colors_bar = ['#FF9800', '#2196F3', '#4CAF50', '#E91E63']
    bars = ax.bar(range(len(names)), vals, color=colors_bar, alpha=0.85)
    ax.set_xticks(range(len(names))); ax.set_xticklabels(names, fontsize=10, rotation=15, ha='right')
    ax.set_ylabel('Full Match'); ax.set_title('Phase 40: The Full Antifragile Pipeline')
    ax.set_ylim(0, 1)
    for b, v in zip(bars, vals):
        ax.text(b.get_x()+b.get_width()/2, v+0.02, f'{v:.1%}', ha='center', fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase40_pipeline.png'), dpi=150)
    plt.close()
