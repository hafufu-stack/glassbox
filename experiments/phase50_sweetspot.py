"""
Phase 50: Ablation Sweet Spot (My own addition)
==================================================
We know 15% is good. But is it THE best?
Fine-grained search: 5%, 8%, 10%, 12%, 14%, 15%, 16%, 18%, 20%, 25%

Also: does the optimal rate depend on the adaptation steps?
Test: 15% with 25, 50, 75, 100 steps.

This finds the exact Pareto frontier of (ablation rate, adaptation steps).
"""
...
def main():
    print("=" * 60)
    print("Phase 50: Ablation Sweet Spot")
    print("Finding the exact optimal (rate, steps) Pareto frontier")
    print("=" * 60)
    t0 = time.time()
    ...
    # Experiment 1: Fine-grained rate sweep (fixed 50 steps)
    rates = [0.0, 0.05, 0.08, 0.10, 0.12, 0.14, 0.15, 0.16, 0.18, 0.20, 0.25]
    results_rate = {}

    print(f"\n  --- Rate Sweep (50 steps) ---")
    print(f"  {'Rate':>6} | {'Full Match':>10} | {'Delta':>8}")
    print("  " + "-" * 30)
    ...
    for rate in rates:
        ...
        for task in test_tasks:
            ...
            if rate > 0:
                abl = ablate_least_important(model, rate, aug_samples)
                current = adapt_model(abl, aug_samples, steps=50)
            else:
                current = adapt_model(model, aug_samples, steps=50)
            ok, tot = eval_task(current, task)
            ok_total += ok; n_total += tot
        r = ok_total / max(n_total, 1)
        results_rate[f'rate_{int(rate*100)}'] = r
        delta = r - results_rate.get('rate_0', r)
        print(f"  {rate:>5.0%} | {r:>9.1%} | {delta:>+7.1%}")

    # Experiment 2: Step sweep (fixed 15%)
    steps_list = [10, 25, 50, 75, 100]
    results_steps = {}

    print(f"\n  --- Step Sweep (15% ablation) ---")
    print(f"  {'Steps':>6} | {'Full Match':>10} | {'Delta':>8}")
    print("  " + "-" * 30)
    ...
    for steps in steps_list:
        ...
        for task in test_tasks:
            ...
            aug_samples = [prep(ai, ao) for ai, ao in aug_pairs]
            aug_samples = [s for s in aug_samples if s]
            abl = ablate_least_important(model, 0.15, aug_samples)
            current = adapt_model(abl, aug_samples, steps=steps)
            ok, tot = eval_task(current, task)
            ok_total += ok; n_total += tot
        r = ok_total / max(n_total, 1)
        results_steps[f'steps_{steps}'] = r
        delta = r - results_rate.get('rate_0', r)
        print(f"  {steps:>5} | {r:>9.1%} | {delta:>+7.1%}")

    elapsed = time.time() - t0
    results = {**results_rate, **results_steps, 'elapsed': elapsed, 'timestamp': time.strftime('%Y-%m-%dT%H:%M:%S')}

    with open(os.path.join(RESULTS_DIR, 'phase50_sweetspot.json'), 'w', encoding='utf-8') as f:
    ...
    ax = axes[0]
    x_rates = [int(r * 100) for r in rates]
    y_rates = [results_rate[f'rate_{r}'] for r in x_rates]
    ax.plot(x_rates, y_rates, 'o-', color='#E91E63', linewidth=2, markersize=8)
    ax.axhline(y=results_rate['rate_0'], color='#FF9800', linestyle='--', label=f'Adapt-only ({results_rate["rate_0"]:.1%})')
    ax.set_xlabel('Ablation Rate (%)'); ax.set_ylabel('Full Match')
    ax.set_title('Rate Sweep (50 adapt steps)'); ax.legend(); ax.grid(True, alpha=0.3)
    best_rate = x_rates[np.argmax(y_rates)]
    ax.annotate(f'Best: {best_rate}%', xy=(best_rate, max(y_rates)),
                xytext=(best_rate+3, max(y_rates)-0.05),
                arrowprops=dict(arrowstyle='->', color='red'), fontsize=12, color='red', fontweight='bold')

    ax = axes[1]
    x_steps = steps_list
    y_steps = [results_steps[f'steps_{s}'] for s in steps_list]
    ax.plot(x_steps, y_steps, 's-', color='#2196F3', linewidth=2, markersize=8)
    ax.set_xlabel('Adaptation Steps'); ax.set_ylabel('Full Match')
    ax.set_title('Step Sweep (15% ablation)'); ax.grid(True, alpha=0.3)
    best_step = x_steps[np.argmax(y_steps)]
    ax.annotate(f'Best: {best_step}', xy=(best_step, max(y_steps)),
                xytext=(best_step+10, max(y_steps)-0.05),
                arrowprops=dict(arrowstyle='->', color='blue'), fontsize=12, color='blue', fontweight='bold')

    plt.suptitle('Phase 50: Ablation Sweet Spot', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase50_sweetspot.png'), dpi=150)
    plt.close()
    print(f"\n  Best rate: {best_rate}%  Best steps: {best_step}")
