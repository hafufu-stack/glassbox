"""
Phase 96: Full-Network MCTS Distillation
==========================================
Fix P92's ZS plateau: instead of training only the policy head,
backpropagate MCTS soft-labels through the ENTIRE GNN backbone.
Compare: head-only distill vs full-network distill.
"""
...
class AgentWithPolicy(nn.Module):
    """GNN with integrated policy head for full-network distillation."""
    def __init__(s,hid=64):
        ...
        # Policy head for action selection distillation
...
def mcts_collect_visits(model,task_samples,n_rollouts=8):
    ...
    # Compute value target: best reward observed
    value_target=max(-best_loss,0.0)
    return visit_dist,value_target,best_action

# ========================================
# Full-Network vs Head-Only Distillation
# ========================================
def distill_head_only(model,soft_targets,epochs=5,lr=1e-3):
    """P92 approach: only train policy_head."""
...
def distill_full_network(model,soft_targets,epochs=5,lr=5e-4):
    """AlphaZero approach: train ENTIRE network end-to-end."""
    ...
    for ep in range(epochs):
        ...
        for nf,nn_c,target_dist,value_target in soft_targets:
            ...
            val_target=torch.tensor([[value_target]],device=DEVICE)
            value_loss=F.mse_loss(pred_v,val_target)
            # Combined: policy + value + small L2 reg to prevent catastrophic forgetting
...
def main():
    print("="*60)
    print("Phase 96: Full-Network MCTS Distillation")
    print("Backbone E2E distillation to break ZS plateau")
    print("="*60)
    t0=time.time()
    ...
    # Create TWO copies: one for head-only, one for full-network
    model_head=AgentWithPolicy().to(DEVICE)
    ...
    model_head.eval()
    model_full=copy.deepcopy(model_head)  # Identical starting point
    ...
    full_zs_hist=[zs_base];full_ttt_hist=[ttt_base]

    for iteration in range(N_ITERS):
        print(f"\n  --- Iteration {iteration+1}/{N_ITERS} ---")

        # Collect soft targets from MCTS (shared between both methods)
        soft_targets=[]
        for task in train_tasks[:60]:
            ...
            aug_s=[prep(ai,ao)for ai,ao in aug_pairs];aug_s=[s for s in aug_s if s]
            if not aug_s:continue
            # Use full_network model for MCTS (it evolves more)
            visit_dist,value_target,_=mcts_collect_visits(model_full,aug_s,n_rollouts=8)
            s0=aug_s[0]
            soft_targets.append((s0['nf'],s0['nn'],visit_dist,value_target))
        print(f"    {len(soft_targets)} soft targets collected")

        # Head-only distillation
        ...
        # Full-network distillation
        model_full,loss_f=distill_full_network(model_full,soft_targets,epochs=5,lr=5e-4)
        zs_f,ttt_f=eval_all(model_full,f"Full-net  iter {iteration+1}")
        full_zs_hist.append(zs_f);full_ttt_hist.append(ttt_f)

        print(f"    Loss: head={loss_h:.4f}, full={loss_f:.4f}")

    results={
        'zs_baseline':zs_base,'ttt_baseline':ttt_base,
        'head_only':{'zs':head_zs_hist,'ttt':head_ttt_hist},
        'full_network':{'zs':full_zs_hist,'ttt':full_ttt_hist},
        'best_head_zs':max(head_zs_hist),'best_full_zs':max(full_zs_hist),
        'best_head_ttt':max(head_ttt_hist),'best_full_ttt':max(full_ttt_hist),
        'n_iterations':N_ITERS,
        'elapsed':time.time()-t0,
        'timestamp':time.strftime('%Y-%m-%dT%H:%M:%S'),
    }
    with open(os.path.join(RESULTS_DIR,'phase96_full_distill.json'),'w',encoding='utf-8') as f:
    ...
    ax.plot(iters,full_zs_hist,'s-',lw=3,ms=10,color='#E91E63',label='Full-Network')
    ...
    ax.set_title('Zero-Shot: Head vs Full Distillation',fontweight='bold')
    # TTT comparison
    ax2=axes[1]
    ax2.plot(iters,head_ttt_hist,'o--',lw=2,ms=8,color='#9E9E9E',label='Head-only (P92)')
    ax2.plot(iters,full_ttt_hist,'s-',lw=3,ms=10,color='#4CAF50',label='Full-Network')
    ...
    ax2.set_title('TTT: Head vs Full Distillation',fontweight='bold')
    plt.suptitle('Phase 96: Full-Network MCTS Distillation',fontsize=15,fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR,'phase96_full_distill.png'),dpi=150);plt.close()
    print(f"\nBest Head ZS={results['best_head_zs']:.1%}, Full ZS={results['best_full_zs']:.1%}")
    print(f"Best Head TTT={results['best_head_ttt']:.1%}, Full TTT={results['best_full_ttt']:.1%}")
