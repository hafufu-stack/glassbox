"""
Phase 51: Neuro-Apoptosis (Destruction + Creation Fusion)
===========================================================
Combine the best of both worlds:
  - P50: Ablate 14% (optimal sweet spot)
  - P48: Neurogenesis 15% (constructive plasticity)

The biological brain does BOTH simultaneously during sleep:
  - Apoptosis: weak synapses are pruned
  - Neurogenesis: new neurons sprout in the hippocampus

Can we break 90% by doing both at once?
"""
...
def neuro_apoptosis(model, ablate_rate, neuro_rate, task_samples):
    """The full metabolism: destroy old + create new simultaneously."""
    am = ablate_least_important(model, ablate_rate, task_samples)
    am = neurogenesis(am, neuro_rate)
    return am
...
def main():
    print("=" * 60)
    print("Phase 51: Neuro-Apoptosis")
    print("Destruction + Creation = The Full Metabolism")
    print("=" * 60)
    t0 = time.time()
    ...
    configs = [
        ('adapt_100',       'Adapt 100 (baseline)',                 0,    0,    100),
        ('ablate14_100',    'Ablate 14% + Adapt 100',               0.14, 0,    100),
        ('neuro15_100',     'Neurogenesis 15% + Adapt 100',         0,    0.15, 100),
        ('apoptosis_14_15', 'Apoptosis (14%+15%) + Adapt 100',     0.14, 0.15, 100),
        ('apoptosis_14_10', 'Apoptosis (14%+10%) + Adapt 100',     0.14, 0.10, 100),
        ('apoptosis_14_20', 'Apoptosis (14%+20%) + Adapt 100',     0.14, 0.20, 100),
        ('apoptosis_10_15', 'Apoptosis (10%+15%) + Adapt 100',     0.10, 0.15, 100),
        ('apoptosis_18_15', 'Apoptosis (18%+15%) + Adapt 100',     0.18, 0.15, 100),
    ]
    results = {}

    print(f"\n  {'Method':<40} | {'Match':>7} | {'Delta':>7}")
    print("  " + "-" * 60)

    for key, label, abl_r, neuro_r, steps in configs:
        ...
        for task in test_tasks:
            ...
            current = model
            if abl_r > 0 and neuro_r > 0:
                current = neuro_apoptosis(current, abl_r, neuro_r, aug_samples)
            elif abl_r > 0:
        ...
        results[key] = rate
        delta = rate - results.get('adapt_100', rate)
        print(f"  {label:<40} | {rate:>6.1%} | {delta:>+6.1%}")

    elapsed = time.time() - t0
    results['elapsed'] = elapsed
    results['timestamp'] = time.strftime('%Y-%m-%dT%H:%M:%S')

    with open(os.path.join(RESULTS_DIR, 'phase51_apoptosis.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    fig, ax = plt.subplots(figsize=(16, 6))
    names = [c[1] for c in configs]
    vals = [results[c[0]] for c in configs]
    colors = ['#9E9E9E', '#E91E63', '#4CAF50', '#673AB7', '#FF9800', '#2196F3', '#00BCD4', '#9C27B0']
    ...
    ax.set_ylabel('Full Match'); ax.set_title('Phase 51: Neuro-Apoptosis (Destruction + Creation)')
    ax.set_ylim(0, 1)
    for b, v in zip(bars, vals):
        ax.text(b.get_x()+b.get_width()/2, v+0.02, f'{v:.1%}', ha='center', fontsize=10, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase51_apoptosis.png'), dpi=150)
    plt.close()
