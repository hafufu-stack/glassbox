"""
Phase 23: Neural + Verify Hybrid
===================================
Combine Phase 21's adapted model with Phase 20's execution verification.
Predict -> Execute -> Verify on demos -> Accept or reject.
"""
...
def execute_op(grid, op, c1, c2):
    arr=np.array(grid).copy()
    if op==1: return arr.tolist()
    elif op==2: arr[:]=c1; return arr.tolist()
    elif op==5: arr[arr==c1]=c2; return arr.tolist()
    elif op==6: return np.flipud(arr).tolist()
    elif op==7: return np.fliplr(arr).tolist()
    elif op==4: m1,m2=arr==c1,arr==c2; arr[m1]=c2; arr[m2]=c1; return arr.tolist()
    return arr.tolist()
...
def main():
    print("="*60); print("Phase 23: Neural + Verify Hybrid"); print("="*60)
    ...
    # Train
    print("\n--- Training ---")
    model=Agent().to(DEVICE); params=sum(p.numel() for p in model.parameters())
    print(f"Params: {params:,}"); opt=torch.optim.Adam(model.parameters(),lr=1e-3)
    for ep in range(80):
    ...
    strategies = {
        'greedy': {'correct':0,'total':0},
        'adapt_10': {'correct':0,'total':0},
        'adapt_10+verify': {'correct':0,'total':0}
    }
    ...
    for task in test_tasks:
        ...
        test_samples=[(prep(p['input'],p['output']),p['input'],p['output']) for p in tests]
        test_samples=[(s,i,o) for s,i,o in test_samples if s is not None]

        for s,inp,out in test_samples:
            ...
            if (ol.argmax(1).item()==s['op'].item() and cl1.argmax(1).item()==s['c1'].item() and
                ...
                strategies['greedy']['correct']+=1

            # Strategy 2: Adapt 10 steps
            ...
            if demo_samples:
                ...
                for _ in range(10):
                    tl=sum(F.cross_entropy(am(d['nf'].to(DEVICE),d['nn'].to(DEVICE))[0],d['op'].to(DEVICE))+
                           ...
                           for d in demo_samples)/len(demo_samples)
                    ao.zero_grad(); tl.backward(); ao.step()
            am.eval()
            with torch.no_grad():
                ol2,cl12,cl22,pl2=am(s['nf'].to(DEVICE),s['nn'].to(DEVICE))
            strategies['adapt_10']['total']+=1
            if (ol2.argmax(1).item()==s['op'].item() and cl12.argmax(1).item()==s['c1'].item() and
                ...
                strategies['adapt_10']['correct']+=1

            # Strategy 3: Adapt + Verify
            strategies['adapt_10+verify']['total']+=1
            op_pred=ol2.argmax(1).item(); c1_pred=cl12.argmax(1).item(); c2_pred=cl22.argmax(1).item()
            # Verify on all demos
            verified=True
            for dp in demos:
                try:
                    result=execute_op(dp['input'],op_pred,c1_pred,c2_pred)
                    if not np.array_equal(np.array(result),np.array(dp['output'])):
                        verified=False; break
                except: verified=False; break
            if verified:
                try:
                    final=execute_op(inp,op_pred,c1_pred,c2_pred)
                    if np.array_equal(np.array(final),out_arr):
                        strategies['adapt_10+verify']['correct']+=1
                except: pass

    for name,data in strategies.items():
        rate=data['correct']/max(data['total'],1)
        print(f"  {name}: {rate:.1%} ({data['correct']}/{data['total']})")

    elapsed=time.time()-t0
    out={k:{'rate':v['correct']/max(v['total'],1),'correct':v['correct'],'total':v['total']}
         for k,v in strategies.items()}
    out['elapsed']=elapsed; out['timestamp']=time.strftime('%Y-%m-%dT%H:%M:%S')
    with open(os.path.join(RESULTS_DIR,'phase23_hybrid.json'),'w',encoding='utf-8') as f:
        json.dump(out,f,indent=2,ensure_ascii=False)

    fig,ax=plt.subplots(figsize=(9,5))
    names=['Greedy','Adapt-10','Adapt-10\n+Verify']
    vals=[strategies[k]['correct']/max(strategies[k]['total'],1) for k in strategies]
    bars=ax.bar(names,vals,color=['#FF9800','#2196F3','#4CAF50'])
    ax.set_ylim(0,1); ax.set_ylabel('Full Match')
    ax.set_title('Phase 23: Neural + Verify Hybrid')
    for b,v in zip(bars,vals): ax.text(b.get_x()+b.get_width()/2,b.get_height()+0.02,f'{v:.1%}',ha='center',fontsize=13)
    plt.tight_layout(); plt.savefig(os.path.join(FIGURES_DIR,'phase23_hybrid.png'),dpi=150); plt.close()
