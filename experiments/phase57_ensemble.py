"""
Phase 57: Ensemble of Diverse Perturbations (My addition)
============================================================
From the P54 grid, multiple configs score high:
  - A=14%, N=0%  → 87.4%
  - A=25%, N=5%  → 87.4%
  - A=25%, N=10% → 88.5%
  - A=25%, N=15% → 87.4%

What if we run K different perturbation configs and MAJORITY VOTE?
Diversity in perturbation → diversity in solutions → ensemble power.
"""
...
def predict_task(model, task):
    """Return predictions as tuples for voting."""
    preds = []
    for tp in task.get('test', []):
        ...
        with torch.no_grad():
            ol, cl1, cl2, pl = model(s['nf'].to(DEVICE), s['nn'].to(DEVICE))
        preds.append((ol.argmax(1).item(), cl1.argmax(1).item(),
                      cl2.argmax(1).item(), pl.argmax(1).item()))
    return preds
...
def main():
    print("=" * 60)
    print("Phase 57: Ensemble of Diverse Perturbations")
    print("Majority vote across multiple perturbation configs")
    print("=" * 60)
    t0 = time.time()
    ...
    # Top configs from P54 grid
    ensemble_configs = [
        ('A25_N10', 0.25, 0.10),  # 88.5%
        ('A14_N0',  0.14, 0.00),  # 87.4%
        ('A25_N5',  0.25, 0.05),  # 87.4%
        ('A25_N15', 0.25, 0.15),  # 87.4%
        ('A14_N5',  0.14, 0.05),  # 87.4%
    ]

    # First, individual scores for reference
    print(f"\n  --- Individual Scores ---")
    individual_results = {}
    for name, abl_r, neuro_r in ensemble_configs:
        ...
        individual_results[name] = rate
        print(f"  {name:<12}: {rate:.1%}")

    # Now ensemble with majority voting
    ensemble_sizes = [3, 5]
    results = dict(individual_results)

    print(f"\n  --- Ensemble (Majority Vote) ---")
    for k in ensemble_sizes:
        selected = ensemble_configs[:k]
        ok_total, n_total = 0, 0
        ...
        for task in test_tasks:
            ...
            all_preds = []
            for name, abl_r, neuro_r in selected:
                current = model
                if abl_r > 0:
                    current = ablate_least_important(current, abl_r, aug_samples)
                if neuro_r > 0:
                    current = neurogenesis(current, neuro_r)
                current = adapt_model(current, aug_samples, steps=100)
                preds = predict_task(current, task)
                all_preds.append(preds)

            # Majority vote per test example
            for ti, tp in enumerate(task.get('test', [])):
                s = prep(tp['input'], tp['output'])
                if s is None: continue
                n_total += 1
                gt = (s['op'].item(), s['c1'].item(), s['c2'].item(), s['ptr'].item())

                votes = [ap[ti] for ap in all_preds if ti < len(ap) and ap[ti] is not None]
                if votes:
        ...
        rate = ok_total / max(n_total, 1)
        results[f'ensemble_{k}'] = rate
        best_individual = max(individual_results[c[0]] for c in selected)
        delta = rate - best_individual
        print(f"  Top-{k} ensemble: {rate:.1%} (best individual: {best_individual:.1%}, delta: {delta:+.1%})")

    elapsed = time.time() - t0
    results['elapsed'] = elapsed
    results['timestamp'] = time.strftime('%Y-%m-%dT%H:%M:%S')

    with open(os.path.join(RESULTS_DIR, 'phase57_ensemble.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    fig, ax = plt.subplots(figsize=(12, 6))
    names = list(individual_results.keys()) + [f'Ensemble-{k}' for k in ensemble_sizes]
    vals = list(individual_results.values()) + [results[f'ensemble_{k}'] for k in ensemble_sizes]
    colors = ['#2196F3'] * len(individual_results) + ['#E91E63'] * len(ensemble_sizes)
    ...
    ax.set_ylabel('Full Match'); ax.set_title('Phase 57: Ensemble of Diverse Perturbations')
    ax.set_ylim(0.7, 1.0)
    for b, v in zip(bars, vals):
        ax.text(b.get_x()+b.get_width()/2, v+0.005, f'{v:.1%}', ha='center', fontsize=10, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase57_ensemble.png'), dpi=150)
    plt.close()
