"""
Phase 46: Double Super-Compensation (My own addition)
=======================================================
What if we STACK both improvements?
  1. First: Knowledge Distillation to create a stronger base (P44)
  2. Then: Apply One-Punch (ablate 15% + adapt 50) at test time (P45)

Hypothesis: Distilled model already encodes "super-compensation wisdom",
so applying another round of ablation+adapt on TOP of it creates
a "double super-compensation" effect — learning from trauma twice.

Counter-hypothesis: The distilled model may already be near optimal,
making further ablation harmful (diminishing returns).

This experiment determines: are the two improvements ADDITIVE or REDUNDANT?
"""
...
def main():
    print("=" * 60)
    print("Phase 46: Double Super-Compensation")
    print("Distill + One-Punch: Additive or Redundant?")
    print("=" * 60)
    t0 = time.time()
    ...
    # Train original base model
    print("\n  Training base model...")
    base_model = Agent().to(DEVICE)
    opt = torch.optim.Adam(base_model.parameters(), lr=1e-3)
    for ep in range(80):
        base_model.train(); random.shuffle(all_samples); el, nb = 0, 0
        for s in all_samples:
            loss = compute_loss(base_model, s)
            opt.zero_grad(); loss.backward(); opt.step()
            el += loss.item(); nb += 1
        if (ep + 1) % 20 == 0:
            print(f"  Epoch {ep+1}/80: loss={el/nb:.4f}")

    # Knowledge Distillation (compact version of P44)
    print("\n  Generating soft labels + training distilled model...")
    T = 2.0
    soft_labels = []
    for task in train_tasks:
        ...
        ablated = ablate_least_important(base_model, 0.15, aug_samples)
        teacher = adapt_model(ablated, aug_samples, steps=50)
        teacher.eval()
        for p in demos:
            s = prep(p['input'], p['output'])
            if s is None: continue
            with torch.no_grad():
                ol, cl1, cl2, pl = teacher(s['nf'].to(DEVICE), s['nn'].to(DEVICE))
                soft_labels.append((s, F.softmax(ol/T, -1).cpu(), F.softmax(cl1/T, -1).cpu(),
                                   F.softmax(cl2/T, -1).cpu(), F.softmax(pl/T, -1).cpu()))
    print(f"  Soft labels: {len(soft_labels)}")

    distilled = Agent().to(DEVICE)
    opt_d = torch.optim.Adam(distilled.parameters(), lr=1e-3)
    for ep in range(80):
        distilled.train(); random.shuffle(soft_labels); el, nb = 0, 0
        for s, s_op, s_c1, s_c2, s_ptr in soft_labels:
            hard = compute_loss(distilled, s)
            ol, cl1, cl2, pl = distilled(s['nf'].to(DEVICE), s['nn'].to(DEVICE))
            soft = (F.kl_div(F.log_softmax(ol/T, -1), s_op.to(DEVICE), reduction='batchmean') +
                    F.kl_div(F.log_softmax(cl1/T, -1), s_c1.to(DEVICE), reduction='batchmean') +
                    F.kl_div(F.log_softmax(cl2/T, -1), s_c2.to(DEVICE), reduction='batchmean') +
                    F.kl_div(F.log_softmax(pl/T, -1), s_ptr.to(DEVICE), reduction='batchmean')) * T * T
            loss = 0.5 * hard + 0.5 * soft
            opt_d.zero_grad(); loss.backward(); opt_d.step()
            el += loss.item(); nb += 1
        if (ep + 1) % 20 == 0:
            print(f"  KD Epoch {ep+1}/80: loss={el/nb:.4f}")

    # ===== 2x3 comparison matrix =====
    print("\n  --- Full 2x3 Comparison Matrix ---")
    configs = [
        ('base_greedy',       'Base Greedy',         base_model, False, False),
        ('base_adapt',        'Base + Adapt 50',     base_model, True,  False),
        ('base_punch',        'Base + One-Punch',    base_model, True,  True),
        ('kd_greedy',         'KD Greedy',           distilled,  False, False),
        ('kd_adapt',          'KD + Adapt 50',       distilled,  True,  False),
        ('kd_punch',          'KD + One-Punch',      distilled,  True,  True),
    ]
    results = {}

    for key, label, m, do_adapt, do_ablate in configs:
        ...
        for task in test_tasks:
            ...
            aug_samples = [s for s in aug_samples if s]
            if do_ablate:
                current = ablate_least_important(m, 0.15, aug_samples)
                current = adapt_model(current, aug_samples, steps=50)
            elif do_adapt:
                current = adapt_model(m, aug_samples, steps=50)
            else:
                current = m
        ...
        results[key] = rate
        print(f"  {label:<30}: {rate:.1%}")

    # Analysis: are they additive?
    base_gain_punch = results['base_punch'] - results['base_adapt']
    kd_gain_punch = results['kd_punch'] - results['kd_adapt']
    kd_gain_base = results['kd_adapt'] - results['base_adapt']
    double_gain = results['kd_punch'] - results['base_adapt']

    print(f"\n  --- Additivity Analysis ---")
    print(f"  One-Punch gain on base:      {base_gain_punch:+.1%}")
    print(f"  One-Punch gain on KD:        {kd_gain_punch:+.1%}")
    print(f"  KD gain over base (adapt):   {kd_gain_base:+.1%}")
    print(f"  Double gain (KD+Punch):      {double_gain:+.1%}")
    print(f"  Sum of individual gains:     {base_gain_punch + kd_gain_base:+.1%}")
    additive = abs(double_gain - (base_gain_punch + kd_gain_base)) < 0.03
    print(f"  Verdict: {'ADDITIVE' if additive else 'REDUNDANT (subadditive)'}")

    elapsed = time.time() - t0
    results['analysis'] = {
        'punch_gain_base': base_gain_punch,
        'punch_gain_kd': kd_gain_punch,
        'kd_gain': kd_gain_base,
        'double_gain': double_gain,
        'additive': additive,
    }
    results['elapsed'] = elapsed
    results['timestamp'] = time.strftime('%Y-%m-%dT%H:%M:%S')

    with open(os.path.join(RESULTS_DIR, 'phase46_double.json'), 'w', encoding='utf-8') as f:
    ...
    colors = ['#9E9E9E', '#FF9800', '#E91E63', '#CE93D8', '#4CAF50', '#673AB7']
    bars = ax.bar(range(6), vals, color=colors, alpha=0.85)
    ax.set_xticks(range(6)); ax.set_xticklabels(labels_plot, fontsize=10)
    ax.set_ylabel('Full Match')
    ax.set_title('Phase 46: Double Super-Compensation (KD + One-Punch)')
    ax.set_ylim(0, 1)
    for b, v in zip(bars, vals):
        ax.text(b.get_x()+b.get_width()/2, v+0.02, f'{v:.1%}', ha='center', fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase46_double.png'), dpi=150)
    plt.close()
