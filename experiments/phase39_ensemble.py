"""
Phase 39: Ablation-Diverse Ensemble
=======================================
Instead of noise-based diversity (SNN-Synthesis), use DESTRUCTION
as the source of diversity. Create K differently-ablated models,
adapt each, and pick the best via demo verification.

For hard tasks (where P38 triggers super-compensation):
  1. Create K=5 ablated copies (different random seeds + strategies)
  2. Adapt each independently (50 steps)
  3. Verify each on demos
  4. Pick the one with best demo performance

This is "Ablation-Diverse Ensemble" — each wound heals differently,
creating diverse recovery paths that break out of local optima.
"""
...
def verify_on_demos(model, demo_samples):
    model.eval()
    ok, total, total_loss = 0, 0, 0.0
    for s in demo_samples:
        if s is None: continue
        total += 1
        with torch.no_grad():
            ol, cl1, cl2, pl = model(s['nf'].to(DEVICE), s['nn'].to(DEVICE))
            loss = (F.cross_entropy(ol, s['op'].to(DEVICE)) +
        ...
        if (ol.argmax(1).item() == s['op'].item() and
            ...
            ok += 1
    return ok, total, total_loss / max(total, 1)


def create_ablated_variants(model, rate, aug_samples, K=5):
    """Create K differently-ablated model variants.
    Each uses a different random seed for ablation mask,
    creating diverse recovery paths."""
    variants = []

    # Variant 0: Strategic (least-important) ablation
    am = copy.deepcopy(model); am.train()
    total_loss = torch.tensor(0.0, device=DEVICE)
    for s in aug_samples[:8]:
        total_loss = total_loss + compute_loss(am, s)
    total_loss = total_loss / max(len(aug_samples[:8]), 1)
    total_loss.backward()
    with torch.no_grad():
    ...
    am.eval()
    variants.append(('strategic', am))

    # Variants 1 to K-1: Random ablation with different seeds
    for i in range(1, K):
        am = copy.deepcopy(model)
        torch.manual_seed(i * 12345 + int(time.time()) % 10000)
        with torch.no_grad():
            for p in am.parameters():
                mask = torch.rand_like(p) > rate
                p.mul_(mask.float())
        variants.append((f'random_{i}', am))

    return variants


def ablation_diverse_ensemble(model, task, aug_samples, demo_samples, K=5, rate=0.15):
    """The Ablation-Diverse Ensemble: create K ablated variants,
    adapt each, pick the one with best demo performance."""

    # Create K ablated variants
    variants = create_ablated_variants(model, rate, aug_samples, K)

    # Adapt each and verify on demos
    best_model = None
    best_ok = -1
    best_loss = float('inf')
    best_variant = 'none'

    for name, ablated in variants:
        adapted = adapt_model(ablated, aug_samples, steps=50)
        ok, total, loss = verify_on_demos(adapted, demo_samples)

        # Pick best: most correct first, then lowest loss as tiebreaker
        if ok > best_ok or (ok == best_ok and loss < best_loss):
            best_ok = ok
            best_loss = loss
            best_model = adapted
            best_variant = name

    return best_model, best_variant, best_ok
...
def main():
    print("=" * 60)
    print("Phase 39: Ablation-Diverse Ensemble")
    print("Destruction as the source of diversity!")
    print("=" * 60)
    t0 = time.time()
    ...
    # Baseline: single ablation + adapt
    ok_single, n_single = 0, 0
    for task in test_tasks:
        demos = task.get('train', [])
        ...
        aug_samples = [prep(ai, ao) for ai, ao in aug_pairs]
        aug_samples = [s for s in aug_samples if s]
        best_m, _, _ = ablation_diverse_ensemble(model, task, aug_samples, demo_samples, K=1)
        ok, tot = eval_task(best_m, task)
        ok_single += ok; n_single += tot
    single_rate = ok_single / max(n_single, 1)
    results['K1'] = single_rate
    print(f"  K=1 (single ablation): {single_rate:.1%}")
    ...
    for K in K_values[1:]:
        ...
        for task in test_tasks:
            ...
            best_m, best_v, _ = ablation_diverse_ensemble(model, task, aug_samples, demo_samples, K=K)
            variant_wins[best_v] = variant_wins.get(best_v, 0) + 1
            ok, tot = eval_task(best_m, task)
            ok_ens += ok; n_ens += tot

        ens_rate = ok_ens / max(n_ens, 1)
        results[f'K{K}'] = ens_rate
        delta = ens_rate - base_rate
        print(f"  K={K} ensemble:         {ens_rate:.1%} (delta: {delta:+.1%})")
        print(f"    Variant wins: {dict(sorted(variant_wins.items(), key=lambda x: -x[1]))}")

    elapsed = time.time() - t0
    results['elapsed'] = elapsed
    results['timestamp'] = time.strftime('%Y-%m-%dT%H:%M:%S')

    with open(os.path.join(RESULTS_DIR, 'phase39_ensemble.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    # Plot
    fig, ax = plt.subplots(figsize=(10, 6))
    all_keys = ['adapt_only', 'K1'] + [f'K{k}' for k in K_values[1:]]
    all_names = ['Adapt\nOnly', 'K=1'] + [f'K={k}' for k in K_values[1:]]
    all_vals = [results.get(k, 0) for k in all_keys]
    colors_bar = ['#9E9E9E', '#FF9800'] + ['#4CAF50'] * len(K_values[1:])
    bars = ax.bar(range(len(all_keys)), all_vals, color=colors_bar, alpha=0.85)
    ax.set_xticks(range(len(all_keys))); ax.set_xticklabels(all_names, fontsize=11)
    ax.set_ylabel('Full Match'); ax.set_title('Ablation-Diverse Ensemble: More Wounds = More Paths')
    ax.set_ylim(0, 1)
    for b, v in zip(bars, all_vals):
        ax.text(b.get_x()+b.get_width()/2, v+0.02, f'{v:.1%}', ha='center', fontsize=12, fontweight='bold')
    ax.axhline(y=base_rate, color='#FF9800', linestyle='--', alpha=0.5, label=f'Adapt baseline ({base_rate:.1%})')
    ax.legend(fontsize=10)

    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase39_ensemble.png'), dpi=150)
    plt.close()
