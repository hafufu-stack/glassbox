"""
Phase 71: Adaptation Learning Rate Sensitivity
================================================
We always use lr=1e-2 for SGD adaptation.
Combined with L2 ablation, what's the optimal LR?
Sweep: 1e-3, 5e-3, 1e-2, 5e-2, 1e-1 with L2 ablation at 15%.
3 seeds.
"""
...
def main():
    print("=" * 60)
    print("Phase 71: Adaptation LR Sensitivity")
    print("What LR works best with L2 ablation?")
    print("=" * 60)
    t0 = time.time()
    ...
    N_SEEDS = 3
    all_results = {f'lr{lr}': [] for lr in lrs}
    ...
    for seed in range(N_SEEDS):
        ...
        for lr in lrs:
            ...
            for task in test_tasks:
                ...
                current = ablate_l2(model, 0.15, aug_samples)
            ...
            all_results[f'lr{lr}'].append(acc)
            print(f"    lr={lr:<8}: {acc:.1%}")

    # Summary
    results = {}
    print(f"\n  === LR SENSITIVITY ({N_SEEDS} seeds) ===")
    best_mean, best_lr = 0, 0
    for lr in lrs:
        key = f'lr{lr}'
        ...
        print(f"  lr={lr:<8}: {mean:.1%} +/- {std:.1%}")
        if mean > best_mean:
            best_mean = mean; best_lr = lr
    ...
    with open(os.path.join(RESULTS_DIR, 'phase71_lr.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    # Plot
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    means = [results[f'lr{lr}_mean'] for lr in lrs]
    stds = [results[f'lr{lr}_std'] for lr in lrs]
    ax.errorbar(range(len(lrs)), means, yerr=stds, marker='o', capsize=4, linewidth=2, color='#4CAF50')
    ax.set_xticks(range(len(lrs))); ax.set_xticklabels([str(lr) for lr in lrs])
    best_idx = lrs.index(best_lr)
    ax.axvline(x=best_idx, color='#E91E63', linestyle='--', alpha=0.7, label=f'Best={best_lr}')
    ax.set_xlabel('Learning Rate'); ax.set_ylabel('Full Match')
    ax.set_title('Phase 71: Adaptation LR with L2 Ablation'); ax.legend()
    ax.grid(True, alpha=0.3); ax.set_ylim(0.7, 1.0)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, 'phase71_lr.png'), dpi=150)
    plt.close()

    print(f"\n  *** OPTIMAL LR: {best_lr} ({best_mean:.1%}) ***")
